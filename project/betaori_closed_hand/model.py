...
class BetaoriClosedHandModel(Model):
    ...
    def calculate_predictions(self, model, epoch):
        ...
        for f in data_files_temp:
            ...
            for prediction in predictions:
                ...
                if 0 < waits_in_hand != len(defending_hand_unique):
                    ...
                    min_wait_position_in_hand_coefficient = pos_in_hand / (
                        len(defending_hand_unique) - waits_in_hand
                    )
                    i_jap += 1
                ...
                if self.print_predictions and printed_results < results_to_print:
                    logger.info("============================================")
                    logger.info(
                        "defending_hand: {}".format(
                            TilesConverter.to_one_line_string([x * 4 for x in defending_hand])
                        )
                    )
                    ...
                    if melds:
                        logger.info(
                            "melds: {}".format(
                                " ".join([TilesConverter.to_one_line_string(x) for x in melds])
                            )
                        )
                    logger.info("waits: {}".format(TilesConverter.to_one_line_string(waits)))
                    logger.info(
                        "tiles_by_danger: {}".format(self.tiles_34_to_sting_unsorted(tiles_by_danger))
                    )
                ...
                sum_max_wait_position += (max_wait_pos - num_waits) / (
                    BetaoriClosedHandProtocol.tiles_unique - num_waits
                )
                sum_avg_wait_position += (avg_wait_pos - avg_pos_offset) / (
                    BetaoriClosedHandProtocol.tiles_unique - num_waits
                )
        ...
        avg_min_wait_position = sum_min_wait_position / total
        avg_max_wait_position = sum_max_wait_position / total
        avg_avg_wait_position = sum_avg_wait_position / total
        avg_genbutsu_error = sum_genbutsu_error / total
        avg_min_wait_position_in_hand = sum_min_wait_position_in_hand / total_jap
        ...
        if epoch:
            ...
            self.graphs_data["second"].append({"epoch": epoch, "avg_genbutsu_error": avg_genbutsu_error})
